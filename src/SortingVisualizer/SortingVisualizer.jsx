  ...
  bubbleSort() {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (isColorChange) {
        ...
        const arrayOfElems = animations[i]
        for (let index = 0; index < arrayOfElems.length; index++) {
          setTimeout(() => {
            const [barOneIdx, newHeight] = arrayOfElems[index];
            const barOneStyle = arrayBars[barOneIdx].style;
            barOneStyle.height = `${newHeight}px`;
          }, i * ANIMATION_SPEED_MS);
        }
    ...
    return (
      <React.Fragment>
        <div className="text-center">
          ...
          <span className='buttonDiv'>
            <button className='btn btn-success' onClick={() => this.testSortingAlgorithms()}>
